...
export async function POST(request:Request){
 try{const body=await request.json() as Record<string,string>;if(!body.name?.trim()||!body.email?.trim()||!body.pageId)return Response.json({error:"Name, email and landing page are required"},{status:400});
  ...
  const response=await proxySocialRequest("/routine-leads",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({routine:"landing_page_registration",externalEventId,name:body.name.trim(),email:body.email.trim(),phone:body.phone?.trim()||null,instagram:body.instagram?.trim()||body.social?.trim()||null,facebook:body.facebook?.trim()||null,x:body.x?.trim()||null,source:"Landing Page",landingPageId:body.pageId,sourceDetail:`landing_page:${body.pageId}`})});
