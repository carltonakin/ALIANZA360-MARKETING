...
| Landing page | `POST/PUT /content` | `dbo.LandingPages` | campaign/title/slug/headline/teaser/status→existing columns; Post URL Link→existing WebinarUrl; payment link→PaymentUrl; video source/player URL/provider and Cloudinary identifiers→Video*/Cloudinary*; CTA→PreVideoCta*; submit text→SubmitButtonText |
