...
| Landing page | `POST/PUT /content` | `dbo.LandingPages` | campaign/title/slug/headline/teaser/webinar/payment/status→existing columns; video source/player URL/provider and Cloudinary identifiers→Video*/Cloudinary*; CTA→PreVideoCta*; submit text→SubmitButtonText |
...
| Webinar registration | `POST /routine-leads` | `dbo.Leads` + `dbo.SocialAccounts` + `dbo.LeadRoutineEvents` | contact and optional Facebook/Instagram/X handles→Leads; nonblank handles→normalized SocialAccounts identities; routine, external event and landing/campaign attribution→LeadRoutineEvents |
