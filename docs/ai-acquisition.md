...
The server normalizes the result, applies confidence/handoff rules, validates the proposed response through communication policy, and persists it as `PROPOSED` or `BLOCKED`. Autonomous selling stops in `HUMAN_HANDOFF`.

## Prospect-to-Lead conversion

Prospects remain in `AIAcquisitionProspects` until explicit conversion after response/engagement or Landing Page registration (unless the configuration relaxes that rule), the configured fit threshold, and do-not-contact checks. Conversion calls `CRMLead_UpsertFromRoutine` using `acquisition-prospect:<id>`, retaining existing email/phone/social matching. It then links `ConvertedLeadId`, links acquisition conversations, adds a conversion activity and summaries of received/sent messages to the existing Lead 360 activity timeline, and calls `LeadScore_Recalculate`. Full conversation text and AI proposals remain in the acquisition conversation table.
...
Authenticated Next.js requests under `/api/acquisition/*` proxy to matching listener routes for configurations/action, discovery, prospects/import/contact/convert, overview, source/channel settings, and analytics. Worker claim/complete and inbound-webhook routes are intentionally unavailable through the browser-facing proxy; integrations call the service-authenticated Express listener directly.

The timer currently runs discovery for active configurations. Automatic initial-outreach/follow-up scheduling, webhook identity resolution for incoming messages, and a CRM-user notification dispatcher are not included. Incoming messages must be supplied with a known Prospect ID and stable external message ID. The UI shows handoff state, while an external task/notification integration must route manual reviews to a user.
...
The acquisition tests cover configuration validation, stable normalization and contact provenance, channel priority/policy enforcement, daily-limit discovery and deduplication, conversion criteria, opt-out handling, structured AI fallback, and independent listener routes. The normal full regression suite remains the release gate for Campaigns, AI Campaigns, Buffer, Landing Page Studio, social listener, Lead scoring, and Lead 360. Migration 026 has not been applied to the configured live MSSQL instance; apply it in a reviewed deployment and smoke-test the new procedures before enabling a configuration.
