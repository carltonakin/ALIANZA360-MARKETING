...
function storedHttpUrl(value, label) {
  try { return httpUrl(value, label); } catch { return ""; }
}
...
export function normalizeLandingPageBlocks(blocks) {
  ...
  return blocks.map((block, index) => {
    ...
    keys.add(id);
    return {
      id, type, sortOrder: index, enabled: bool(block?.enabled, true), config: normalizeConfig(type, block?.config),
      ...(Number.isSafeInteger(block?.blockId) && block.blockId > 0 ? { blockId: block.blockId } : {}),
    };
  });
}
...
export function legacyLandingPageBlocks(page = {}) {
  ...
  const legacyCtaUrl = storedHttpUrl(page.preVideoCtaUrl, "CTA URL");
  if ((page.preVideoCtaEnabled ?? Boolean(page.preVideoCtaText && legacyCtaUrl)) && page.preVideoCtaText && legacyCtaUrl) {
    blocks.push({ ...defaultLandingPageBlock("CTA_BUTTON", "legacy-cta"), config: { text: text(page.preVideoCtaText, 255), url: legacyCtaUrl, style: "primary", alignment: "left", openInNewTab: false } });
  }
  blocks.push({ ...defaultLandingPageBlock("REGISTRATION_FORM", "legacy-registration"), config: { ...defaultLandingPageBlock("REGISTRATION_FORM").config, submitButtonText: text(page.submitButtonText, 255) || LANDING_PAGE_SUBMIT_TEXT, postSubmitUrl: storedHttpUrl(page.webinarUrl, "Post-registration URL") } });
  const legacyPaymentUrl = storedHttpUrl(page.paymentUrl, "Payment URL");
  if (legacyPaymentUrl) blocks.push({ ...defaultLandingPageBlock("PAYMENT_CTA", "legacy-payment"), config: { ...defaultLandingPageBlock("PAYMENT_CTA").config, url: legacyPaymentUrl } });
...
export function resolveLandingPageBlocks(page = {}) {
  if (Array.isArray(page.blocks)) {
    const blocks = [];
    const seen = new Set();
    for (const block of [...page.blocks].sort((a, b) => Number(a.sortOrder || 0) - Number(b.sortOrder || 0))) {
      try {
        const normalized = normalizeLandingPageBlocks([block])[0];
        if (seen.has(normalized.id)) continue;
        seen.add(normalized.id);
        blocks.push(normalized);
      } catch {
        // An invalid saved block must not hide the rest of the published page.
      }
    }
    return blocks.map((block, sortOrder) => ({ ...block, sortOrder }));
  }
...
export function projectLegacyLandingPageFields(blocks, fallback = {}) {
  ...
  return {
    ...
    mediaOrder: hasPicture && hasVideo && ordered.findIndex((item) => item.enabled && item.type === "IMAGE") < ordered.findIndex((item) => item.enabled && item.type === "VIDEO") ? "PICTURE_FIRST" : "VIDEO_FIRST",
